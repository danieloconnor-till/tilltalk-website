...
export async function POST(request: Request) {
  ...
  if (!railwayUrl || !onboardingKey) {
    return NextResponse.json({ error: 'Railway not configured' }, { status: 500 })
  }

  try {
    const res = await fetch(`${railwayUrl}/api/sandbox/message`, {
      method: 'POST',
    ...
    return NextResponse.json({ error: 'Request to Railway timed out' }, { status: 504 })
  }
}
