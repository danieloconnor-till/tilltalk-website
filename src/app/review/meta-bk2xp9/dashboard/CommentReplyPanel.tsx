  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="mb-3">
        ...
        <p className="text-xs text-gray-600 mt-2 leading-relaxed">
          Click &ldquo;Load latest comment&rdquo; to see the comment this reply
          will answer.
        </p>
      </div>
      ...
      <div className="mb-3">
        ...
        >
          {loadingPreview ? 'Loading…' : 'Load latest comment'}
        </button>
      </div>
        ...
        preview.ok ? (
          <div className="mb-3 rounded-lg border border-blue-300 bg-blue-50 p-4">
            ...
            <p className="text-sm text-blue-900 whitespace-pre-wrap break-words mb-2">
              {preview.inbound.text ?? '(no text in comment)'}
            </p>
            <p className="text-[11px] text-blue-700 font-mono break-all">
              Target comment: {preview.inbound.comment_id} (post{' '}
              {preview.inbound.post_id})
            </p>
          </div>
        ) : (
          <div className="mb-3 rounded-lg border border-amber-300 bg-amber-50 p-4">
            ...
            {preview.error === 'no_recent_comment' || preview.status === 409 ? (
              <p className="text-xs text-amber-900">
                Comment on a Page post from another account first, then click
                &ldquo;Load latest comment&rdquo; again.
              </p>
            ) : null}
          </div>
        )
